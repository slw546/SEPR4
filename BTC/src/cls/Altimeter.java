...
/**
 * Shows the aircraft's height in feet (whatever units you want).
 * And the current banking of the aircraft.
 * @author Huw Taylor
 */
public class Altimeter implements EventHandler {
	...
	public void mouseReleased(int key, int mx, int my) {
		...
		if (key == input.MOUSE_LEFT
				&& (currentAircraft.status() == AirportState.NORMAL)) {
			if (mouseOverTopButton(mx, my)
					&& (currentAircraft.position().z() < 30000)) {
				currentAircraft.setAltitudeState(AltitudeState.CLIMBING);
			} else if (mouseOverBottomButton(mx, my)
					&& (currentAircraft.position().z() > 28000)) {
				currentAircraft.setAltitudeState(AltitudeState.FALLING);
			}
		}
	}
	 ...
	 * Icon depicts aircraft's orientation
	 */
	private void drawAircraftIcon() {
		// angle to draw the aircraft
		double r = 0;
