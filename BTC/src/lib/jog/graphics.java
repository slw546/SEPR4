...
public abstract class graphics {
	...
	/**
	 * Draws a scaled texture image at the specified coordinates
	 * The coordinates are also scaled as to ensure the image is drawn 
	 * in the same position on smaller windows
	 * @param drawable - the image to be drawn
	 * @param x - the horizontal pixel to draw at.
	 * @param y - the vertical pixel to draw at.
	...
	static public void drawScaled(Image drawable, double x, double y, double scale) {
		...
		/*drawable.x = x * scale; //Set the x-coordinate of the image drawn by that specified by the x parameter
		y = (window.height() - y);
		drawable.y = y * scale; //Set the y-coordinate of the image drawn by that specified by the y parameter
		double w = drawable.scaledWidth(scale);
		double h = -drawable.scaledHeight(scale);*/
