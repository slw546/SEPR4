...
public class Demo extends Scene {
	...
	public void start() {
		...
		ordersBox = new cls.OrdersBox(ORDERSBOX_X, ORDERSBOX_Y,
	...
	public void update(double dt) {
		...
		for (Aircraft plane : aircraftInAirspace) {
			plane.update(dt);
			if (plane.status() == Aircraft.AirportState.FINISHED){
		...
		for (int i = aircraftInAirspace.size()-1; i >=0; i --) {
			if (aircraftInAirspace.get(i).status() == Aircraft.AirportState.FINISHED) {
	 ...
	 * @param a1 the first plane involved in the collision
	 * @param a2 the second plane in the collision
	 */
	public void gameOver(Aircraft a1, Aircraft a2) {
		if (!Main.testing) {
			playSound(audio.newSoundEffect("sfx" + File.separator + "crash.ogg"));
		}

		main.closeScene();
		main.setScene(new GameOver(main, a1, a2, 0));
	}
	...
	public void mouseReleased(int key, int x, int y) {
		...
		Aircraft.AltitudeState altitudeState = Aircraft.AltitudeState.LEVEL;
		if (selectedAircraft != null) {
	...
	private Aircraft createAircraft() {
		...
		int o = (new Random()).nextInt((locationWaypoints.length - 1 - 0) + 1) + 0;
		int d = (new Random()).nextInt((locationWaypoints.length - 1 - 0) + 1) + 0;
		while (LOCATION_NAMES[d] == LOCATION_NAMES[o]){
			d = (new Random()).nextInt((locationWaypoints.length - 1 - 0) + 1) + 0;
		}
	...
	private boolean aircraftSelectableAtAltitude(Aircraft a, int altitude) {
		...
		if (a.position().z() < altitude && a.altitudeState() == Aircraft.AltitudeState.CLIMBING) return true;
		if (a.position().z() > altitude && a.altitudeState() == Aircraft.AltitudeState.FALLING) return true;
		return false;
	}
